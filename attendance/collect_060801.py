...
log_file = "e:\\atten\\"+datetime.datetime.now().strftime('%Y%m')+".log"
...
def getrecord(machineID,machineIP,machineLOC,days,db,data_tbl,log_tbl):
    """
    ...
    if not zk.Connect_Net(machineIP, 4370):
        statement0 = "insert into " + log_tbl + " values(sysdate," + "'!!!" + str(machineLOC) + str(machineIP)+"连接失败.')"
        db.cursor().execute(statement0)
    ...
    statement = "insert into " \
                  + data_tbl \
                  + " values(seq_rec_tmp.nextval," \
                  + ":machine_id,:emp_id,:verify_mode,:in_out_mode,:rec_data,:rec_time,:work_code," \
                  + "sysdate)"
    db.cursor().executemany(statement, rec)
    statement1 = "insert into " + log_tbl + " values(sysdate," + "'" + str(machineLOC) + str(machineIP)+"采集成功.')"
    db.cursor().execute(statement1)
    db.commit()
    zk.Disconnect()
    logging.info(str(machineLOC) + str(machineIP)+"采集成功")
    return 1
...
if __name__ == '__main__':
    ...
    for row in m:
        ...
        machineLOC = row[2]
        #读取考勤记录，成功返回1
        success = getrecord(machineID, machineIP, machineLOC, DAYS, db, DATA_TBL, LOG_TBL)+success
    cursor.callproc('proc_hr_imp_record')
    logging.info("采集完毕，本次共采集"+str(len(m))+"台，成功"+str(success)+ "台，失败"+str(len(m)-success)+"台。")
    logging.info("数据请查看" + str(DATA_TBL) + "表，错误日志请查看"+ str(LOG_TBL) + "表。")
    db.close()
